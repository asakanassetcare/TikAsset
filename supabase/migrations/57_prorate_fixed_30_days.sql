-- Migration 57: Fix prorated rent calculation
-- 1. Use contract_start_date for billing (not actual_move_in_at which is physical move-in)
-- 2. Use fixed 30-day month divisor instead of actual days in month
...
BEGIN
  ...
  -- Use contract_start_date for billing; actual_move_in_at is staff reference only
  v_move_in := v_contract.contract_start_date;
  ...
  -- Use fixed 30-day month for rate calculation
  v_prorated      := ceil(v_contract.monthly_rent * v_days_charged / 30.0);
  ...
  VALUES (v_invoice_id,
          ...
          'rent', v_days_charged,
          round(v_contract.monthly_rent / 30.0, 2),
          v_prorated, 1);
  ...
  LOOP
    v_addon_prorated := ceil(v_addon.amount * v_days_charged / 30.0);
    INSERT INTO invoice_items(invoice_id, description, item_type, quantity, unit_price, amount, display_order)
    VALUES (v_invoice_id, v_addon.name || ' (prorated)', 'addon', v_days_charged,
            round(v_addon.amount / 30.0, 2), v_addon_prorated, 10);
  END LOOP;
  ...
  IF (SELECT total_amount FROM invoices WHERE id = v_invoice_id) <= 0 THEN
    ...
    VALUES (v_invoice_id, v_applied, current_date, 'pending_approve',
            'ค่าทำสัญญาล่วงหน้า', v_slip_url, auth.uid());
  END IF;
